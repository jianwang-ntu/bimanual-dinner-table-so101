...
With no policy supplied the arms simply hold their home pose, so the task score
is expected to be zero on every seed.  What this run *does* establish is that
each randomized episode is a valid, solvable starting point: objects are on the
table, inside an arm's reach, not interpenetrating, and the simulation is
stable for the full episode.  That is the evaluation harness the policy will
later be scored by, exercised end to end.

Run:  python3 scripts/eval_seeds.py --seeds 10
"""
...
def episode(seed: int, seconds: float, render: pathlib.Path | None) -> dict:
...
def main() -> int:
    ...
    for s in range(args.seeds):
        ...
        row = episode(s, args.seconds, png)
        rows.append(row)
    ...
    out = {
        ...
        "policy": None,
        "policy_note": "No policy is loaded. The arms hold the home pose, so a "
                       "task score of 0 is the expected and correct result; "
                       "this run scores the ENVIRONMENT, not a controller.",
        "all_episodes_valid_and_stable": ok,
        "episodes": rows,
    }
    EVID.mkdir(parents=True, exist_ok=True)
    (EVID / "eval_seeds.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
    print(f"\n{args.seeds} seeds: "
          f"{'all valid and stable' if ok else 'FAILURES PRESENT'} "
          f"-> evidence/eval_seeds.json")
